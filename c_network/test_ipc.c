// Ce fichier est un programme de test pour vérifier le bon fonctionnement de la
// communication inter-processus (IPC). Il permet d'écrire ou de lire des
// données factices dans la mémoire partagée.
...
static void afficher(const GameState *s) {
  ...
  for (int i = 0; i < s->unit_count; i++) {
    ...
    printf("  [%d] team=%d peer=%d pos=(%.1f,%.1f) hp=%d/%d %s%s%s\n", u->id,
           u->team, u->owner_peer, u->x, u->y, u->hp, u->hp_max,

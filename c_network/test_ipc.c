 ...
 *               ou : gcc -Wall -g -I../shared -o test_ipc test_ipc.c ipc.c -lrt
 * -lpthread
 */
...
static void afficher_etat(const GameState *s, const char *titre) {
  ...
  printf("  magic      = 0x%08X  %s\n", s->magic,
         s->magic == PROTOCOL_MAGIC ? "[OK]" : "[ERREUR! magic invalide]");
  printf("  version    = %d\n", s->version);
  printf("  peer_id    = %d\n", s->my_peer_id);
  printf("  tick       = %u\n", s->tick);
  printf("  unites     = %d\n\n", s->unit_count);
  ...
  printf("  %-4s  %-5s  %-5s  %-6s  %-6s  %-6s  %-6s  %-5s  %-5s\n", "ID",
         "team", "peer", "x", "y", "hp", "hp_max", "alive", "dirty");
  printf("  %-4s  %-5s  %-5s  %-6s  %-6s  %-6s  %-6s  %-5s  %-5s\n", "----",
         "-----", "-----", "------", "------", "------", "------", "-----",
         "-----");
...
static int mode_ecriture(int my_peer_id) {
  ...
  printf("     c_net va maintenant envoyer les unites dirty (id=0, id=1) en "
         "UDP.\n");
...
static int mode_boucle(void) {
  ...
  while (1) {
    ...
    if (ipc_read_state(&state) == 0) {
      ...
      printf("\r[it=%d] tick=%-6u unites=%-3d dont %d reseau    ", ++iteration,
             state.tick, state.unit_count, nb_reseau);
      fflush(stdout);

// Ce fichier gère la mémoire partagée et les sémaphores pour la communication
// inter-processus (IPC). Il permet de lire et d'écrire l'état du jeu pour le
// partager entre les différents composants.
...
int ipc_init(const char *shm_name, const char *sem_w, const char *sem_r,
             int create) {
  if (create) {
    shm_unlink(shm_name);
    sem_unlink(sem_w);
  ...
  g_shm = mmap(NULL, sizeof(GameState), PROT_READ | PROT_WRITE, MAP_SHARED,
               g_fd, 0);
  if (g_shm == MAP_FAILED) {
    perror("mmap");
    close(g_fd);
